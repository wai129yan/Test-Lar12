...
@section('contact')
    <h1 class="text-3xl font-bold text-center hel">Categories Create</h1>
    ...
    <div class="w-100 bg-slate-100 m-auto p-7 mt-5 rounded-2xl">
        <form class="max-w-sm mx-auto" action="{{ route('categories.store') }}" method="post">
            ...
            <div class="mb-5">
                <label for="is_active" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Active</label>
                <select id="is_active" name="is_active" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="draft" {{ old('is_active', 'draft') == 'draft' ? 'selected' : '' }}>Draft</option>
                    <option value="published" {{ old('is_active') == 'published' ? 'selected' : '' }}>Published</option>
                </select>
                @error('is_active')
                    <div class="text-red-500 mt-1">{{ $message }}</div>
                @enderror
